	            ...
	            function processHaat2(haatData, ht_str) {
					...
					pg_query(q, vals, function(pg_err, pg_rows, pg_res){
						...
						return res.send({
							...
							FROM " + pg_schema + ".fm_contours a, " + pg_schema + ".amr_interfering_contours b \
...
function fmForAvailableChannel(req, res) {
	...
	try {
	    ...
	    var q = "WITH tmp_table as \
			...
			FROM " + pg_schema + ".fm_contours a, tmp_table b \
...
function allAMCallsignList(req, res) {
	...
	try {
		var q = "SELECT distinct callsign FROM " + pg_schema + ".am_contours ORDER BY callsign";
		var vals = [];
